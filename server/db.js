...
export async function initDb() {
  ...
  try {
    ...
    // Actualizar publicaciones existentes con los nuevos textos ricos
    for (const art of DEFAULT_ARTICLES) {
      ...
      if (exists.rows.length === 0) {
        ...
        console.log(`✅ Artículo sembrado: ${art.slug}`);
      } else {
        await client.query(`
          UPDATE posts SET 
            title = $1, read_time = $2, summary = $3, highlights = $4::jsonb, 
            metrics_table = $5::jsonb, sections = $6::jsonb, recommendation = $7, accent = $8, accent_bg = $9
          WHERE slug = $10
        `, [
          art.title, art.read_time, art.summary, art.highlights,
          art.metrics_table, art.sections, art.recommendation, art.accent, art.accent_bg,
          art.slug
        ]);
        console.log(`🔄 Artículo actualizado con información extendida: ${art.slug}`);
      }
    }

    console.log('✅ Base de datos inicializada y sincronizada con contenido enriquecido.');
  } catch (err) {
